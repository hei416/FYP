...
export default function AI({ showChat, setShowChat, setSelectedPdf, setTargetPage }) {
    ...
    // Navigate to /history, passing current chat session
    const handleEnlarge = () => {
        if (history.length > 0) {
            const sessionId = Date.now().toString(36);
            const session = {
                id: sessionId,
                title: history.find(m => m.role === 'user')?.content?.slice(0, 50) || 'Chat session',
                createdAt: Date.now(),
                messages: history
            };
            // Merge into existing saved sessions
            const existing = JSON.parse(localStorage.getItem('codetutor_chat_history') || '[]');
            localStorage.setItem('codetutor_chat_history', JSON.stringify([session, ...existing]));
            sessionStorage.setItem(SESSION_KEY, JSON.stringify({ id: sessionId }));
        }
        setShowChat(false);
        navigate('/history');
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${API_BASE}/ragAI`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ user_input: userInput, history: [...history, userMessage] }),
            });
                                        ...
                                        {chunkContext.chunks.map((chunk, idx) => {
                                            ...
                                            return (
                                                <div key={idx} style={{ padding: 14, marginBottom: 10, backgroundColor: isTarget ? "#fef3c7" : "#ffffff", border: isTarget ? "3px solid #f59e0b" : "1px solid #e5e7eb", borderRadius: 6, fontSize: 13, lineHeight: 1.8, whiteSpace: "pre-wrap", fontFamily: "Georgia, serif", position: "relative" }}>
                                                    {isTarget && (<div style={{ position: "absolute", top: -10, left: 10, backgroundColor: "#f59e0b", color: "white", padding: "3px 10px", borderRadius: 4, fontSize: 10, fontWeight: "bold", fontFamily: "system-ui" }}>⭐ RETRIEVED CHUNK</div>)}
                                                    <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 6, fontFamily: "system-ui", fontStyle: "italic" }}>{isTarget ? "→ This paragraph was used in the answer" : idx < chunkContext.target_index ? "↑ Previous chunk" : "↓ Next chunk"}</div>
                                                    {chunk}
                                                </div>
                                            );
                                        })}
            ...
            {showChat && (
                <div style={{ position: "fixed", top: 64, right: 20, width: 800, height: "75vh", backgroundColor: colors.surface, boxShadow: shadows.lg, borderRadius: radii.lg, zIndex: 10000, display: "flex", flexDirection: "column" }}>
                    ...
                    <div style={{ padding: spacing.lg, borderBottom: `1px solid ${colors.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", backgroundColor: colors.accent, borderRadius: `${radii.lg}px ${radii.lg}px 0 0`, color: colors.surface }}>
                        ...
                        <div style={{ display: 'flex', gap: spacing.sm }}>
                            <button
                                ...
                                style={{ padding: "6px 12px", cursor: "pointer", borderRadius: radii.sm, border: "none", backgroundColor: colors.surface, color: colors.accent, fontWeight: font.weightSemibold }}
                                title="Open full conversation history page"
                            >
                                ⬆ Enlarge
                            </button>
                    ...
                    <div style={{ flex: 1, padding: spacing.lg, overflowY: "auto", backgroundColor: colors.bg }}>
                        {history.length === 0 && (
                            <div style={{ textAlign: 'center', marginTop: '30%', color: colors.textMuted }}>
                                <div style={{ fontSize: 32, marginBottom: 8 }}>☕</div>
                                <div style={{ fontSize: font.sizeMd, color: colors.textSecondary }}>Ask me anything about Java!</div>
                                <div style={{ fontSize: font.sizeXs, color: colors.textMuted, marginTop: 4 }}>Press ⬆ Enlarge for full conversation history</div>
                            </div>
                        )}
                        {history.map((msg, idx) => (
