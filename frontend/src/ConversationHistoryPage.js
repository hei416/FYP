...
export default function ConversationHistoryPage() {
    ...
    }, [sessions, activeId, loadingSessionId]);
   ...
   const deleteSession = async (id, e) => {
        ...
        const isLocalOnly = !session?.isDb && !id.includes('-');  // UUIDs have hyphens, local IDs don't
        ...
        <div style={{ display: 'flex', height: 'calc(100vh - 64px)', background: colors.bg, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '32px 10%' }}>
                    {/* Empty state */}
                    {(!activeSession || (activeSession && (activeSession.messages?.length ?? 0) === 0 && loadingSessionId !== activeId )) && (
                ...
                <div style={{
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            disabled={loadingSessionId !== null || !userInput.trim()}
                            style={{
                                ...btn.accent, ...btn.small,
                                opacity: (loadingSessionId === null && userInput.trim()) ? 1 : 0.45,
                                cursor: (loadingSessionId === null && userInput.trim()) ? 'pointer' : 'not-allowed',
                            }}
                        >
                            {loadingSessionId !== null ? '⏳' : 'Send'}
                        </button>
                    </form>
