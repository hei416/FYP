...
function ClassroomQuizManager({ classroomId, analysisData }) {
  ...
  const [genPrompt, setGenPrompt] = useState('Describe the topics or concepts to test in the exercise');
  ...
  const [genSource, setGenSource] = useState('classroom'); // 'classroom' | 'course' | 'both'
  ...
  const DEFAULT_PROMPT = 'Describe the topics or concepts to test in the exercise';
  ...
  // Load topic groups when source='course'/'both' and course path changes
  useEffect(() => {
    if (genSource !== 'course' && genSource !== 'both') return;
  ...
  useEffect(() => {
    if ((genSource !== 'course' && genSource !== 'both') || genSelectedTopics.length === 0) return;
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await generateClassroomQuiz(classroomId, {
        ...
        file_ids: (genSource === 'classroom' || genSource === 'both') && genFileIds.length > 0 ? genFileIds : null,
        source: genSource,
        course_path: (genSource === 'course' || genSource === 'both') ? genCoursePath : null,
      });
  ...
  const handleEditExisting = (quiz) => {
    ...
    setEditAttemptLimit(quiz.attempt_limit || null);
  ...
  const handleSaveQuiz = async (publishStatus) => {
    ...
    try {
      ...
      setGenPrompt('Describe the topics or concepts to test in the exercise');
      setGenTitle('');
      setGenSectionId('');
      setGenFileIds([]);
      ...
      {showGenForm && !draftQuestions && (
        <div style={{
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {(genSource === 'course' || genSource === 'both') && (
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <textarea
              ...
              placeholder="Based on the source, give me 10 quiz questions"
              rows={2}
          ...
          {/* File picker — only shown for classroom document source (or 'both') */}
          {(genSource === 'classroom' || genSource === 'both') && <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
      ...
      {draftQuestions && (
        <div style={{ ...card.base, padding: 20, marginBottom: 28, border: `2px solid ${colors.primary}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 12, marginBottom: 20 }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: font.sizeXs, fontWeight: font.weightSemibold, color: colors.textSecondary, marginBottom: 4 }}>Max Attempts</label>
              <input
                type="number"
                min={1}
                max={99}
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 24px' }}>
      ...
      {activeTab === 'official-lessons' && classroomView === 'quizzes' && !quizzesLoading && (!quizzesData || quizzesData.length === 0) && (
        <div style={{ ...card.base, textAlign: 'center', padding: '56px 32px' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>📝</div>
          <div style={{ fontSize: font.sizeLg, fontWeight: font.weightSemibold, color: colors.textSecondary, marginBottom: 8 }}>No exercises created yet</div>
          <div style={{ fontSize: font.sizeSm, color: colors.textMuted }}>Create exercises in the Quizzes & Tests tab</div>
        </div>
      )}
...
function ClassroomPracticalChallengeManager({ classroomId, analysisData }) {
  ...
  const [genSectionId, setGenSectionId] = useState('');
  const [genSource, setGenSource] = useState('classroom'); // 'classroom' | 'course' | 'both'
  const [genCoursePath, setGenCoursePath] = useState('basic_java');
  const [genFileIds, setGenFileIds] = useState([]);   // selected file IDs for context
  const [genTopicGroups, setGenTopicGroups] = useState([]);
  const [genSelectedTopics, setGenSelectedTopics] = useState([]);
  const [generating, setGenerating] = useState(false);
  const [genError, setGenError] = useState(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [challengeData, secData, fileData] = await Promise.all([
  ...
  // Load attempt data for each challenge
  useEffect(() => {
    if (!classroomId || challenges.length === 0) return;
    ...
    const loadAttemptData = async () => {
      ...
      for (const challenge of challenges) {
        try {
          const results = await getClassroomPracticalChallengeStudentResults(classroomId, challenge.id);
          
          // Count total attempts and passed
          let totalAttempts = 0;
          let passedCount = 0;
          
          if (Array.isArray(results)) {
            // results is array of student results
            totalAttempts = results.length;
            passedCount = results.filter(r => r.passed).length;
          } else if (results && Array.isArray(results.attempts)) {
            // Fallback for different response format
            totalAttempts = results.attempts.length;
            passedCount = results.attempts.filter(a => a.passed).length;
          }
          
          attemptMap[challenge.id] = {
            totalAttempts,
            passedCount,
            studentResults: results,
          };
        } catch (err) {
          console.warn(`Failed to load attempts for challenge ${challenge.id}:`, err);
          attemptMap[challenge.id] = { totalAttempts: 0, passedCount: 0, studentResults: [] };
        }
      }
      
      setChallengeAttempts(attemptMap);
    };
    
    loadAttemptData();
  }, [classroomId, challenges]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div>
      ...
      {showGenForm && !draftData && (
        <div style={{ ...card.base, padding: 20, marginBottom: 24, border: `1px solid ${colors.primary}`, background: '#f0f7ff' }}>
              ...
              <div style={{ marginTop: 10 }}>
                ...
                {genTopicGroups.length > 0 && (
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                      <span style={{ fontSize: font.sizeXs, fontWeight: font.weightSemibold, color: colors.textSecondary }}>
                        Topics <span style={{ fontWeight: 400, color: colors.textMuted }}>(select to filter; prompt auto-filled)</span>
                      </span>
                      <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                        <button type="button"
                          onClick={() => setGenSelectedTopics(genTopicGroups.map(g => g.label))}
                    ...
                    <div style={{ maxHeight: 160, overflowY: 'auto', border: `1px solid ${colors.border}`, borderRadius: radii.sm, background: colors.bg, padding: '4px 8px' }}>
                      {genTopicGroups.map(g => {
                        const checked = genSelectedTopics.includes(g.label);
                        return (
                          <label key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 4px', borderRadius: radii.sm, cursor: 'pointer', background: checked ? '#eff6ff' : 'transparent', marginBottom: 1 }}>
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => setGenSelectedTopics(prev =>
          ...
          {/* File picker — only shown for classroom document source (or 'both') */}
          {(genSource === 'classroom' || genSource === 'both') && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                <label style={{ fontSize: font.sizeXs, fontWeight: font.weightSemibold, color: colors.textSecondary }}>
                  Source Files (optional)
                ...
                <span style={{ fontSize: font.sizeXs, color: colors.textMuted }}>
                  — leave unchecked to search all documents
                </span>
                {allFiles.length > 0 && (
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                    <button type="button" onClick={() => setGenFileIds(allFiles.map(f => f.id))}
                      style={{ fontSize: 11, padding: '2px 8px', border: `1px solid ${colors.border}`, borderRadius: radii.sm, cursor: 'pointer', background: colors.bg, color: colors.textSecondary }}>Select All</button>
                    <button type="button" onClick={() => setGenFileIds([])}
                      style={{ fontSize: 11, padding: '2px 8px', border: `1px solid ${colors.border}`, borderRadius: radii.sm, cursor: 'pointer', background: colors.bg, color: colors.textSecondary }}>Clear</button>
                  </div>
                )}
              </div>
              <div style={{ maxHeight: 140, overflowY: 'auto', border: `1px solid ${colors.border}`, borderRadius: radii.sm, background: colors.bg, padding: '6px 8px' }}>
                {allFiles.length === 0 ? (
                  <div style={{ padding: '8px', fontSize: font.sizeXs, color: colors.textMuted, fontStyle: 'italic' }}>
                    No files uploaded yet. Upload files in the Learning Materials tab first.
                  </div>
                ) : allFiles.map(f => {
                  const checked = genFileIds.includes(f.id);
                  return (
                    <label key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 6px', borderRadius: radii.sm, cursor: 'pointer', background: checked ? '#eff6ff' : 'transparent', marginBottom: 2 }}>
                      <input type="checkbox" checked={checked}
                        onChange={() => setGenFileIds(ids => ids.includes(f.id) ? ids.filter(id => id !== f.id) : [...ids, f.id])}
                        style={{ accentColor: colors.primary, flexShrink: 0 }} />
                      <span style={{ fontSize: font.sizeXs, color: checked ? colors.primary : colors.text, fontWeight: checked ? font.weightSemibold : 'normal', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        📄 {f.filename}
                      </span>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea
              value={genPrompt}
              onChange={e => setGenPrompt(e.target.value)}
              placeholder="e.g. Inheritance and polymorphism with abstract classes"
              rows={2}
              style={{ width: '100%', padding: '8px 10px', border: `1px solid ${colors.border}`, borderRadius: radii.sm, fontSize: font.sizeSm, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }}
            />
          </div>
